...
(async () => {
  ...
  const [activeColumns] = await db.promise().query("SHOW COLUMNS FROM customers LIKE 'is_active'");
  if (!activeColumns.length) await db.promise().query('ALTER TABLE customers ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1');
  await db.promise().query('UPDATE customers SET is_salesman = 1, is_active = 1 WHERE id = ? AND email = ?', [5, email]);
  const [enabled] = await db.promise().query('SELECT id, is_salesman, is_active FROM customers WHERE id = ?', [5]);
