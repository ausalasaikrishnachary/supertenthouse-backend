...
function fixture(users) {
  const queries = [];
  const db = { promise: () => ({ query: async (sql, args) => { queries.push(sql); return [users.filter(user => user.email === args[0] && user.is_salesman === 1)]; } }) };
...
test('enabled customer receives a salesman token with the same customer ID', async () => {
  const f = fixture([{ id: 5, email: 'fixture@example.invalid', password: await bcrypt.hash('fixture-password', 4), is_salesman: 1 }]);
  ...
  assert.match(f.queries[0], /FROM customers WHERE email = \? AND is_salesman = 1/);
});
test('ordinary customers, wrong passwords and missing passwords cannot log in as salesmen', async () => {
  for (const [permission, saved, supplied] of [[0, 'correct', 'correct'], [1, 'correct', 'wrong'], [1, null, 'anything']]) {
    const f = fixture([{ id: 5, email: 'fixture@example.invalid', password: saved, is_salesman: permission }]);
...
test('legacy password works without rewriting the customer record', async () => {
  const f = fixture([{ id: 5, email: 'fixture@example.invalid', password: 'fixture', is_salesman: 1 }]);
