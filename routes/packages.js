...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [req.params.id], (err, results) => {
    ...
    // Fetch linked product_ids
    db.query("SELECT product_id FROM package_products WHERE package_id = ?", [pkg.id], (prodErr, prodResults) => {
      if (prodErr) {
        console.error("Error fetching package products:", prodErr);
        pkg.product_ids = [];
      } else {
        pkg.product_ids = prodResults.map(r => r.product_id);
      }
      res.json(pkg);
    });
  });
...
// ====================================
router.get("/:id/add_ons", (req, res) => {
  const packageId = req.params.id;

  // First check if category column exists
  db.query("SHOW COLUMNS FROM add_ons LIKE 'category'", (colErr, columns) => {
    ...
    let sql = `
      SELECT 
        a.*,
        pa.is_default
      FROM (SELECT id, addon_name AS name, price, icon, description, category, is_active FROM add_ons) a INNER JOIN package_addons pa ON a.id = pa.addon_id
...
// ====================================
router.post("/:id/add_ons", (req, res) => {
  const packageId = req.params.id;
...
// ====================================
router.patch("/:packageId/add_ons/:addonId/default", (req, res) => {
  ...
  try {
    ...
    db.query(sql, values, (err, result) => {
      ...
      if (req.body.product_ids) {
        try {
          parsedProductIds = typeof req.body.product_ids === 'string' ? JSON.parse(req.body.product_ids) : req.body.product_ids;
        } catch (e) {
          parsedProductIds = [];
        }
      }

      const saveProductsAndRespond = () => {
        if (parsedProductIds && parsedProductIds.length > 0) {
          const productValues = parsedProductIds.map(prodId => [result.insertId, prodId]);
          db.query(
            "INSERT INTO package_products (package_id, product_id) VALUES ?",
            [productValues],
            (prodErr) => {
              if (prodErr) {
                console.error("Error adding package products:", prodErr);
              }
              res.json({
    ...
    db.query(
      ...
      (err, results) => {
        ...
        db.query(sql, values, (err) => {
          ...
          db.query(
            "DELETE FROM package_products WHERE package_id = ?",
            [id],
            (prodDeleteErr) => {
              if (prodDeleteErr) {
                console.error("Error deleting package products:", prodDeleteErr);
              }
              ...
              const saveProductsAndRespond = () => {
                if (parsedProductIds && parsedProductIds.length > 0) {
                  const productValues = parsedProductIds.map(prodId => [id, prodId]);
                  db.query(
                    "INSERT INTO package_products (package_id, product_id) VALUES ?",
                    [productValues],
                    (prodErr) => {
...
router.get("/:id/products", (req, res) => {
  const packageId = req.params.id;
  const sql = `
    SELECT p.*, (SELECT image_url FROM product_images WHERE product_id = p.id LIMIT 1) AS image_url
    FROM products p
    INNER JOIN package_products pp ON p.id = pp.product_id
    WHERE pp.package_id = ? AND p.is_active = 1
  `;
  db.query(sql, [packageId], (err, results) => {
    if (err) {
      console.error("Error fetching package products:", err);
      return res.status(500).json({ error: err.message });
    }
    res.json(results);
  });
